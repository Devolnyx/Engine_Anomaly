...
#model = models.load_model('model_overhang/') #4

def load_data(path):
    data = pd.read_csv(path, header = None)
    data = np.array(data[1]).reshape(-1, 1000)
...
def plot_data(true, pred, label):
    ...
    errors = (abs(pd.DataFrame(true) - pd.DataFrame(pred))).mean(axis=1)
        #MAPE
    #errors = (abs((pd.DataFrame(true) - pd.DataFrame(pred))/pd.DataFrame(true))).mean(axis=1)
